...
4. 在codegen.toy中使用新增的BinOp
    ...
    执行以下命令`./build/bin/toyc-ch7 -emit=mlir ./toy_mod/src/Ch7/codegen.toy -opt`可以生成如下MLIR：
...
1. 为新增Op定义Lowering
    ...
    此时执行`./build/bin/toyc-ch7 -emit=mlir-affine ./toy_mod/src/Ch7/codegen.toy`命令生成lowering到affine dialect的mlir时，会报如下错误：

    ```
    loc("./toy_mod/src/Ch7/codegen.toy":18:16): error: 'arith.ori' op operand #0 must be signless-integer-like, but got 'f64'
    ```
    ...
    void ASTDumper::dump(ExprAST *expr) {
      llvm::TypeSwitch<ExprAST *>(expr)
          ...
          .Case<BinaryExprAST, CallExprAST, LiteralExprAST, NumberExprAST,
                ...
                VarIntDeclExprAST, VariableExprAST>([&](auto *node) { this->dump(node); })
          .Default([&](ExprAST *) {
      ...
      ...
        ...
        std::unique_ptr<VarDeclExprAST> parseDeclaration(bool requiresInitializer) {
          ...
          if (lexer.getCurToken() == tok_var)
            ...
            return parseVarIntDeclaration(requiresInitializer);
        ...
        std::unique_ptr<VarIntDeclExprAST> parseVarIntDeclaration(bool requiresInitializer) {
        ...
        std::unique_ptr<ExprASTList> parseBlock() {
          ...
          while (lexer.getCurToken() != '}' && lexer.getCurToken() != tok_eof) {
            ...
            } else if (lexer.getCurToken() == tok_var || lexer.getCurToken() == tok_varint) {
...
4. 通过ODS框架修改现有Op使其兼容整数
    ...
    ```TableGen
    ...
    // 1. 将I64Tensor加到Toy_Type中
    def Toy_Type : AnyTypeOf<[F64Tensor, I64Tensor, Toy_StructType]>;
    ...
    ...
    // 2. 定义整数类型常数操作ConstantIntOp
    def ConstantIntOp : Toy_Op<"constantint", [ConstantLike, Pure,
    ...
    def AndOp : Toy_Op<"and",
      ...
      // 3. 修改AndOp的参数类型： F64Tensor -> I64Tensor
    ...
    def OrOp : Toy_Op<"or",
      ...
      // 4. 修改OrOp的参数类型： F64Tensor -> I64Tensor
      let arguments = (ins I64Tensor:$lhs, I64Tensor:$rhs);
      let results = (outs I64Tensor);
