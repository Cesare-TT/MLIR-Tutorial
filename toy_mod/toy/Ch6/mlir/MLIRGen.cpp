...
private:
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    } else if (callee == "fptoui") {
      /// Emit a FPToUIOp expression.
      if (call.getArgs().size() != 1) {
        emitError(location, "MLIR codegen encountered an error: toy.fptoui "
                            "does not accept multiple arguments");
        return nullptr;
      }
      return builder.create<FPToUIOp>(location, operands[0]); 
    }
