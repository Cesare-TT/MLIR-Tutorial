...
void AndOp::build(mlir::OpBuilder &builder, mlir::OperationState &state,
                  mlir::Value lhs, mlir::Value rhs) {
  state.addTypes(UnrankedTensorType::get(builder.getI64Type()));
  state.addOperands({lhs, rhs});
}
...
void FPToUIOp::build(mlir::OpBuilder &builder, mlir::OperationState &state,
                        mlir::Value value) {
  state.addTypes(UnrankedTensorType::get(builder.getI64Type()));
  state.addOperands(value);
}

void FPToUIOp::inferShapes() { getResult().setType(getResult().getType()); }
