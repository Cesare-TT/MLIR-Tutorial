...
private:
  ...
  std::unique_ptr<ExprAST> parseNumberExpr() {
    auto loc = lexer.getLastLocation();
    auto result =
        std::make_unique<NumberExprAST>(std::move(loc), lexer.getValueType(), lexer.getIntValue(), lexer.getFloatValue());
  ...
  std::unique_ptr<ExprAST> parseTensorLiteralExpr() {
    ...
    return std::make_unique<LiteralExprAST>(std::move(loc), lexer.getValueType(),
                                            std::move(values), std::move(dims));
  }
