...
4. 检查PrintOpLowering的代码会发现，Toy的Example在将PrintOP lowering到LLV吗的printf时，格式标识符固定传了%f。
...
7. 重新编译后，再次执行`./build/bin/toyc-ch6 -emit=jit ./toy_mod/src/Ch6/codegen.toy -opt`命令，可以看到整数类型值已经能够正确的print。
